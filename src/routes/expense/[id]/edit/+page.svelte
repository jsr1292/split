...
<svelte:head>
  <title>Split — Editar gasto</title>
</svelte:head>

<div style="margin-bottom: 12px;">
  <a href="/expense/{e.id}" style="font-size: 10px; color: var(--text3); letter-spacing: 0.05em; display: inline-flex; align-items: center; gap: 4px;">← Volver</a>
</div>

<div class="section-header" style="margin-bottom: 16px;">Editar gasto</div>
  ...
  <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 6px;">
    {#each categoryList as cat}
      <button onclick={() => category = cat.id}
        style="display: flex; flex-direction: column; align-items: center; gap: 2px; padding: 8px 4px; border-radius: 6px; border: 1px solid {category === cat.id ? 'var(--gold)' : 'var(--border)'}; background: {category === cat.id ? 'rgba(201,168,76,0.1)' : 'var(--bg2)'}; cursor: pointer; color: {category === cat.id ? 'var(--gold)' : 'var(--text3)'}; font-family: inherit; font-size: 9px;">
        <span style="font-size: 18px;">{cat.emoji}</span><span>{cat.label}</span>
      </button>
    {/each}
  </div>
</div>
